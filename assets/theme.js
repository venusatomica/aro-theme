...
function applyGradient() {
  ...
  $(".image-gradient").each(function() {
    $(this).addClass(
      "image-gradient--" +
        possibleGradients[randomNumber(possibleGradients.length) - 1]
    );
  });
}
...
$(document).ready(function() {
  updateMinicart = $.debounce(1000, function() {
    return jQuery
      .getJSON("/cart.js", function(cart) {
        ...
        for (var i = 0; i < cart.items.length; i++) {
          ...
          items += createItem(item, i);
        }
        if (items === "") {
          items =
            '<div class="minicart-empty"><div>Sua sacola ainda está vazia :(</div></div>';
        ...
        // Subtotal
        $(".subtotal-value").html(formatPrice(cart.items_subtotal_price));
        $(".subtotal-installments").html(
          getInstallments(cart.items_subtotal_price)
        );

        // Frete
        var isFreteGratis = cart.items_subtotal_price > 39800;
        var freteTexto = isFreteGratis
          ? "Você conseguiu o frete grátis, carai! 🙃"
          : "Frete grátis a partir de R$ 398.";
        $(".minicart-frete").html(freteTexto);

        // Bubble
        $("[data-cart-count]").text(cart.item_count);
      })
      .done(function() {
        $(".minicart-quantity-button-increment").on(
          "click",
          incrementItemQuantity
        );
        $(".minicart-quantity-button-decrement").on(
          "click",
  ...
  if ($(".product-recommendations__inner")) {
...
function handleMinicart() {
  ...
  $(".btn-minicart-dismiss").on("click", function(ev) {
    ev.preventDefault();
    $(".minicart-content").removeClass("minicart-content--open");
  });

  $(".site-header__cart").on("click", function(ev) {
    ev.preventDefault();
    $(".minicart-content").addClass("minicart-content--open");
    updateMinicart();
  });
}
...
function incrementItemQuantity(ev) {
  ev.preventDefault();
  var line = $(this).data("item-line") + 1;
  var quantity = $(this).data("item-quantity");

  updateItemQuantity(line, quantity + 1);
}

function decrementItemQuantity(ev) {
  ev.preventDefault();
  var line = $(this).data("item-line") + 1;
  var quantity = $(this).data("item-quantity");
...
function updateItemQuantity(line, quantity) {
  jQuery
    .ajax({
      type: "POST",
      url: "/cart/change.js",
...
function createItem(product, line) {
  var productName =
    "<div class='minicart-product-name'>" + getProductName(product) + "</div>";
  var productPrice =
    "<div class='minicart-price-item'>" + formatPrice(product.price) + "</div>";
  var productInstallments =
    "<div class='minicart-installments'>" +
    getInstallments(product.price) +
    "</div>";
  var productPriceContainer =
  ...
  var productQuantity =
    ...
    createDecrementQuantity(product, line) +
    product.quantity +
    createIncrementQuantity(product, line) +
    "</div>";
  ...
  var productImage =
    "<div class='minicart-product-image'><img src=" +
    product.image +
    " title='" +
...
function createIncrementQuantity(product, line) {
  return (
    "<button class='btn-link minicart-quantity-button minicart-quantity-button-increment' data-item-line='" +
    line +
    "' data-item-quantity='" +
    product.quantity +
    "'>+</button>"
...
function createDecrementQuantity(product, line) {
  return (
    "<button class='btn-link minicart-quantity-button minicart-quantity-button-decrement' data-item-line='" +
    line +
    "' data-item-quantity='" +
    product.quantity +
    "'>-</button>"
...
function getProductName(product) {
  var options =
    product.options_with_values && product.options_with_values.length > 0
      ? product.options_with_values
          ...
          .join(" ")
      : "";
  return product.product_title + " " + options;
}

function getInstallments(value) {
  var installmentValue = value / 6;
  return "6x " + formatPrice(installmentValue);
}
...
function hideOnClickOutside(element, triggerElement) {
...
theme.menu = (function() {
  $(".header--desktop .sidebar-menu").on("click", function(event) {
    var element = $(".header--desktop .sidebar-menu-wrapper");
    var isVisible = $(element).is(":visible");

    if (isVisible === false) {
      element.show();
      hideOnClickOutside(element[0], $(".header--desktop .sidebar-menu")[0]);
    } else {
      element.hide();
    }
  ...
  $(".header--mobile .sidebar-menu-button").on("click", function(event) {
    var element = $(".header--mobile .sidebar-menu-wrapper");
    var isVisible = $(element).is(":visible");
    ...
    if (isVisible === false) {
      ...
      hideOnClickOutside(
        element[0],
        $(".header--mobile .sidebar-menu-button")[0]
      );
    } else {
      element.hide();
    }
...
theme.menuAllProducts = (function() {
  $(".all-products-button").on("click", function(event) {
    var element = $(".all-products-menu-wrapper");
    var isVisible = $(element).is(":visible");

    if (isVisible === false) {
      element.show();
      hideOnClickOutside(element[0], $(".all-products-button")[0]);
    } else {
      element.hide();
    }
